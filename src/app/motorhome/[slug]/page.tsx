...
import { Calendar, ShieldCheck, Clock, User, Share2, Info, Sun, Battery } from 'lucide-react';
...
export async function generateMetadata({ params }: ReviewPageProps): Promise<Metadata> {
    ...
    const fullTitle = `${post.title} | Guia de Geladeira Motorhome`;
    const fullDescription = post.excerpt || `Guia completo sobre ${post.title} para motorhome e vanlife.`;
    const url = `https://guiadegeladeira.com.br/motorhome/${slug}`;
...
export async function generateStaticParams() {
    const posts = getAllPosts('motorhome');
...
export default async function ReviewPage({ params }: ReviewPageProps) {
    ...
    const jsonLd = {
        ...
        "@graph": [
            {
                "@type": "Article",
                "@id": `https://guiadegeladeira.com.br/motorhome/${slug}#article`,
                "headline": post.title,
                "name": post.title,
                ...
                "dateModified": post.date ? new Date(post.date).toISOString() : new Date().toISOString(), // Fallback to publish date if no modified date
                "author": {
                    "@type": "Person",
                ...
                "mainEntityOfPage": {
                    "@type": "WebPage",
                    "@id": `https://guiadegeladeira.com.br/motorhome/${slug}`
                },
                "isPartOf": {
            ...
            {
                "@type": "BreadcrumbList",
                "@id": `https://guiadegeladeira.com.br/motorhome/${slug}#breadcrumb`,
                "itemListElement": [
                    {
                        "@type": "ListItem",
                        "position": 1,
                    ...
                    },
                    {
                        ...
                        "position": 2,
                        "name": "Motorhome",
                        "item": "https://guiadegeladeira.com.br/motorhome"
                    },
                    {
                        "@type": "ListItem",
                        "position": 3,
        ...
        <main className="min-h-screen bg-slate-50 pb-24">
            ...
            <article className="container mx-auto px-4 max-w-2xl py-8">
                ...
                <nav className="flex items-center gap-2 text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-6">
                    ...
                    <Link href="/motorhome" className="hover:text-slate-600 transition-colors">Motorhome</Link>
                    <span className="opacity-30">/</span>
                ...
                <header className="mb-10 relative">
                    <div className="mb-4 flex flex-wrap items-center gap-4">
                        <span className="bg-teal-100 text-teal-800 text-[10px] font-black px-2 py-1 rounded uppercase tracking-[0.2em] border border-teal-200">
                            Vanlife & Camping
                        </span>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-slate-100 rounded-lg border border-slate-200 inset-metal">
                        <div className="w-10 h-10 bg-teal-900 rounded-full flex items-center justify-center text-amber-400 shadow-lg relative overflow-hidden">
            ...
            prose-li:marker:text-amber-500 prose-img:rounded-xl prose-img:shadow-lg">
                ...
                {/* AUTONOMY FOOTER BOX */}
                <div className="mt-16 relative p-8 bg-teal-900 text-white rounded-2xl overflow-hidden shadow-2xl">
                    <div className="absolute inset-0 bg-diamond-plate opacity-10"></div>
                    <div className="absolute top-0 bottom-0 right-0 w-2 bg-amber-400"></div>

                    <div className="relative z-10">
                        <div className="flex items-center gap-4 mb-6">
                            <div className="w-14 h-14 bg-amber-400 rounded-full flex items-center justify-center text-slate-900 shadow-xl border-4 border-teal-800">
                                <Sun className="w-8 h-8" />
                            </div>
                            <div>
                                <h4 className="font-black font-teko text-2xl uppercase tracking-wider leading-none">Independência Energética</h4>
                                <p className="text-slate-300 text-xs uppercase tracking-widest mt-1">Sua casa, suas regras.</p>
                            </div>
                        </div>

                        <p className="text-slate-300 text-sm leading-relaxed mb-6 font-medium italic">
                            &quot;A liberdade de não precisar de camping começa na escolha dos equipamentos certos. Priorize eficiência e autonomia.&quot;
                        </p>

                        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 bg-teal-950/50 p-3 rounded border border-teal-800">
                            <Battery className="w-4 h-4 text-amber-400 shrink-0" />
                            <span className="uppercase tracking-widest">Dica: Dimensione sempre suas baterias com 30% de margem de segurança.</span>
                        </div>
                    </div>
                </div>
